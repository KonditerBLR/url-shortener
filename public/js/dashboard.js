...
function closeTagSelector() {
    ...
    hideInlineTagCreator();
}

// Show inline tag creator
function showInlineTagCreator() {
    const creator = document.getElementById('inlineTagCreator');
    const btn = document.querySelector('.btn-create-tag-inline');
    creator.style.display = 'block';
    btn.style.display = 'none';
    document.getElementById('newTagNameInline').focus();
}

// Hide inline tag creator
function hideInlineTagCreator() {
    const creator = document.getElementById('inlineTagCreator');
    const btn = document.querySelector('.btn-create-tag-inline');
    creator.style.display = 'none';
    btn.style.display = 'flex';
    document.getElementById('newTagNameInline').value = '';
    document.getElementById('newTagColorInline').value = '#6366f1';
    document.getElementById('newTagColorHexInline').textContent = '#6366f1';
}

// Update color hex display
function updateTagColorHex() {
    const colorInput = document.getElementById('newTagColorInline');
    const hexDisplay = document.getElementById('newTagColorHexInline');
...
async function createAndAttachTag() {
    const nameInput = document.getElementById('newTagNameInline');
    const colorInput = document.getElementById('newTagColorInline');
    const name = nameInput.value.trim();
    const color = colorInput.value;
    ...
    try {
        ...
        if (newTag) {
            ...
            if (success) {
                closeTagSelector();
                await showLinks();
                toast.success(`Tag "${name}" created and attached successfully!`);
            }
        }
    } catch (error) {
    ...
    content.innerHTML = `
        <div class="webhooks-section">
            ...
            <div class="webhook-docs">
                ...
                <div class="webhook-docs-example">
